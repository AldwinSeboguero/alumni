   ...
   <v-card>
    <v-container>
     <v-data-table
        ...
        :server-items-length="totalDeans"
        :items-per-page="10" 
        show-select 
        :footer-props="{
          itemsPerPageOptions: [5, 10, 15],
          itemsPerPageText: 'Dean Per Page',
      ...
      <td>{{deans.data.indexOf(item)+1}}</td> 
    </template>
...
export default {
  ...
  methods: {
     readDataFromAPI() {
      ...
      let pageNumber = page;
      axios
        .get(`/api/v1/deans?page=` + pageNumber, {
          params: { 'per_page': itemsPerPage },
        })
        .then((response) => {
          //Then injecting the result to datatable parameters.
          this.loading = false;
          this.user_dean = response.data.user_dean;
         this.deans = response.data.deans;
          this.campuses = response.data.campuses;
          this.programs = response.data.programs;
          this.sections = response.data.sections;
          this.totalDeans = response.data.deans.total;
          this.numberOfPages = response.data.deans.last_page;
        });
    },
    ...
    searchIt(d) {
      if (d.length > 2) {
        ...
        let pageNumber = page;
        axios
          .get(`/api/v1/deans/${d}?page=` + pageNumber, {
          params: { 'per_page': itemsPerPage },
        })
          .then((res) => {
            this.loading = false;  
            this.deans = res.data.deans;
             this.user_dean = res.data.user_dean;
            console.log(this.deans);
            this.totalDeans = res.data.deans.total;
            this.numberOfPages = res.data.deans.last_page;
          })
          .catch((err) => {
            console.error(err);
          });
    ...
    programListenerNew(){
     ...
     updateDean(item){
      const index = this.deans.data.indexOf(item);
      axios.post('/api/v1/dean/update',{'new_dean': item.user_id, 'dean' : item.id})
      .then(res =>{
        this.deans = res.data.deans;
    ...
    deleteItem(item) {
      ...
      if (decide) {
        axios
          .delete("/api/v1/deans/" + item.id)
          .then((res) => {
            this.text = "Record Deleted Successfully!"; 
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            this.deans.data.splice(index, 1);
          })
          .catch((err) => {
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        axios
          .put("/api/v1/deans/" + this.editedItem.id, this.editedItem)
          .then((res) => {
            this.text = "Record Updated Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            Object.assign(this.deans.data[index], res.data.dean);
            console.log(this.editedItem);
          })
          .catch((err) => {
            console.log(err.response);
        ...
        axios
          .post("/api/v1/deans", this.editedItem)
          .then((res) => {
            this.text = "Record Added Successfully!";
            this.snackbarColor ="primary darken-1";
            this.snackbar = true;
            // this.deans.data.push(res.data.dean); 
            this.deans = res.data.deans
          })
          .catch((err) => {
            console.dir(err);

...
class PdfController extends Controller
{
    public function create(Request $request)
    {
        ...
        $staffREGISTRARSTAFF_id =null;
         //staff id
         if ($staffPD) {
            $staffPD_id = Staff::where('user_id',$staffPD->user_id)->where('semester_id',$activeClearancePurpose->semester_id)->first()->id;
         
         }
         if ($staffDEAN) {
            $staffDEAN_id = Staff::where('user_id',$staffDEAN->user_id)->where('semester_id',$activeClearancePurpose->semester_id)->first()->id;
         
        }
        if ($staffSTCOUNCIL) {
            $staffSTCOUNCIL_id = Staff::where('user_id',$staffSTCOUNCIL->user_id)->where('semester_id',$activeClearancePurpose->semester_id)->first()->id;
         
        }
        if ($staffCASHIER) {
            $staffCASHIER_id = Staff::where('designee_id',Designee::where('name','Cashier')->first()->id)->where('campus_id',$student->program->campus_id)->where('semester_id',$activeClearancePurpose->semester_id)->first()->id;
        
        }
        if ($staffOSAS) {
            $staffOSAS_id = Staff::where('designee_id',Designee::where('short_name','OSAS')->first()->id)->where('campus_id',$student->program->campus_id)->where('semester_id',$activeClearancePurpose->semester_id)->first()->id;
        
        }
        if ($staffLIBRARY) {
            $staffLIBRARY_id = Staff::where('designee_id',Designee::where('name','Library')->first()->id)->where('campus_id',$student->program->campus_id)->where('semester_id',$activeClearancePurpose->semester_id)->first()->id;
         
        }
        if ($staffREGISTRAR) {
            $staffREGISTRAR_id = Staff::where('designee_id',Designee::where('name','Registrar')->first()->id)->where('campus_id',$student->program->campus_id)->where('semester_id',$activeClearancePurpose->semester_id)->first()->id;
        
        }
        if ($staffREGISTRARSTAFF) {
            $staffREGISTRARSTAFF_id = Staff::where('user_id',$staffREGISTRARSTAFF->user_id)->where('semester_id',$activeClearancePurpose->semester_id)->first()->id;
